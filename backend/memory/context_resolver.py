...
class QueryContext:
    ...
    position_selector: str = ""       # Orthogonal scope label ("section 9.2", "the last two"); "" = none
...
class ContextResolver:
    ...
    def resolve(
        ...
        # Position scope (orthogonal to retrieval_intent): the LLM sets position_selector
        # when the query targets a part by place/structure ("section 9.2", "the last two").
        # Trusting the LLM keeps this free of hardcoded label words — it recognises a
        # structural/ordinal reference conceptually, in any phrasing.
        position_selector = str(llm_result.get("position_selector") or "").strip()
    ...
    def _build_segments(
        ...
        Each kept segment is ``{query, intent, files, title, position_selector}``. A
        segment's intent is validated against the retrieval-intent enum (defaulting to
        factual) and its source_files against the real loaded-file list (unknown names
        dropped). Returns a list ONLY when there are >= 2 valid segments; otherwise []
        (single-operation query → normal single-intent path).
        """
        ...
        for seg in segments_raw:
            ...
            position_selector = str(seg.get("position_selector") or "").strip()
            out.append({
                "query": q, "intent": intent, "files": files, "title": title,
                "position_selector": position_selector,
            })
