...
class LLMClassifier:
    ...
    def _parse_llm_response(self, response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # position_selector: orthogonal scope label ("section 9.2", "the last two");
            # "" when the query is not scoped by position/structure. Normalize to a string.
            result["position_selector"] = str(result.get("position_selector") or "").strip()
            ...
            # operation word (an intent name like 'positional'/'comparison') into this field
            # instead of one of the four valid values. An INVALID value is a formatting
            # error, NOT a signal that the query is vague — so it must not become 'ambiguous'
            # (which would wrongly SKIP retrieval). Infer from the structured outputs, and
            # default an otherwise-answerable query to INDEPENDENT so it still retrieves:
            #   - 2+ segments    -> a COMPOUND query (independent drives it via segments)
            #   - 2+ sub_queries -> a MULTI_GROUP query
            #   - otherwise      -> INDEPENDENT (answerable; retrieve rather than skip)
            # (A genuinely vague query is flagged by the model EXPLICITLY emitting the valid
            # value 'ambiguous', which passes through untouched.)
        ...
        for i, sq in enumerate(sub_queries_from_llm):
            ...
            if isinstance(sq, dict):
                ...
                if intent in valid_intents:
                    ...
                    # This subject's OWN position/structural scope, if any. Threaded into
                    # per-subject retrieval so a positionally-named subject ("section 9.2")
                    # is located by a full-document scan, not by similarity.
                    pos = str(sq.get("position_selector") or "").strip()
                    if pos:
                        entry["position_selector"] = pos
                    results[i] = entry
                else:
        ...
        if needs_llm:
            ...
            with ThreadPoolExecutor(max_workers=min(len(needs_llm), 5)) as executor:
                ...
                for future in as_completed(future_to_index):
                    ...
                    if meta.get("position_selector"):
                        entry["position_selector"] = meta["position_selector"]
                    results[idx] = entry
    ...
    def _get_topic_and_intent_for_subquery(self, sub_query: str, index: int, total: int,
        ...
        try:
            ...
            # This subject's OWN position scope, if it named a part by place/structure.
            pos = str(data.get("position_selector") or "").strip()
            if pos:
                result["position_selector"] = pos
