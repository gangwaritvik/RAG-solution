...
class Retriever:  
    ...
    def retrieve(self, query: str, top_k: int = TOP_K, get_all_relevant: bool = False, retrieval_intent: str = None, top_k_override: int = None, retrieve_all: bool = False, restrict_filenames=None, position_scoped: bool = False):
        log.info(f"[RETRIEVER] Query: '{query[:60]}' | top_k: {top_k} | get_all_relevant: {get_all_relevant} | intent: {retrieval_intent} | top_k_override: {top_k_override} | retrieve_all: {retrieve_all} | restrict_filenames: {restrict_filenames} | position_scoped: {position_scoped}")
        ...
        # ── Position scope: when the query targets a specific section/clause/ordinal item
        # (position_selector set by the classifier), the part is located by READING THE
        # WHOLE DOCUMENT IN ORDER, never by similarity — a bare label like "9.2" has almost
        # no semantic signal and would be dropped by any cap. So force an exhaustive scan
        # regardless of the operation intent (summary/comparison/extraction/...). A user
        # Top-K override still wins (handled below) for explicit manual capping. ──
        if position_scoped and not (top_k_override is not None and top_k_override > 0):
            mode = "exhaustive"
            is_unbounded = True
            chunk_threshold = 0.0
            log.info("[RETRIEVER] Position-scoped query → forcing EXHAUSTIVE full-document scan")
