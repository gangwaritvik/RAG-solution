...
from backend.config import UPLOAD_DIR, CHROMA_DIR, FRONTEND_DIR, TOP_K, MAX_UPLOAD_BYTES, MAX_UPLOAD_MB
from backend.utils.logger import get_logger

# pipeline.py is the composition root: importing it runs the module-level
# initialization that wires up every singleton (Embedder, VectorStore, retriever,
# generator, memory manager, ...). app.py reuses those exact instances so the API
# and the rest of the system share one initialized pipeline.
import backend.pipeline as core
        ...
        for uf in files:
            ...
            if name.endswith(".pdf") or name.endswith(".docx") or name.endswith(".doc"):
                ...
                if len(content) > MAX_UPLOAD_BYTES:
                    return _err(
                        f"File '{uf.filename}' exceeds the {MAX_UPLOAD_MB} MB upload limit.",
                        413,
                    )
...
# ──────────────────────────────────────────────────────────────────────
def _select_and_retrieve(query_context, top_k, top_k_override, effective_retrieve_all,
                         get_all_relevant, intent, prev_answer_chunk, DependencyType):
    """Pick the retrieval strategy for a resolved query and execute it (STEP 2).

    Returns ``(hits, segments)`` where ``segments`` is only populated for compound
    (multi-operation) queries. Extracted from ``_resolve_and_retrieve`` so the
    strategy-selection logic can be unit-tested in isolation.
    """
    ...
    if query_context.dependency_type == DependencyType.AMBIGUOUS:
        ...
        for seg in segments:
            seg_hits = core.retriever.retrieve(
                ...
                position_scoped=bool(seg.get("position_selector")),
            )
            seg["hits"] = seg_hits
        ...
        if not hits:
            ...
            hits = core.retriever.retrieve(
                ...
                position_scoped=bool(getattr(query_context, "position_selector", "")),
            )
...
def _resolve_and_retrieve(query: str, top_k: int, group_id, top_k_override, retrieve_all) -> dict:
    ...
    """
    from backend.memory.context_resolver import DependencyType
    ...
    hits, segments = _select_and_retrieve(
        query_context, top_k, top_k_override, effective_retrieve_all,
        get_all_relevant, intent, prev_answer_chunk, DependencyType,
    )
...
def _resolve_query(query: str, top_k: int, temp: float, group_id, top_k_override, retrieve_all):
    ...
    if segments:
        ...
        answer, memory_summary = core.generator.generate(
            ...
            turn_count=ctx["turn_count"],
            position_selector=getattr(query_context, "position_selector", ""),
        )
