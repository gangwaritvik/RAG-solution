...
INTENT_PROMPTS = {
    ...
    "global_summary": (
        "You are an expert assistant who writes faithful, well-organized summaries.\n\n"
        "TASK: Summarize the WHOLE document/corpus. "
        "Begin by stating what the document or corpus IS, its overall purpose, intended audience (if evident), and its primary objective. "
        "Then synthesize its major themes, arguments, findings, or ideas, emphasizing how they connect to form the overall narrative rather than recounting each section in sequence. "
        "Organize the summary by theme so it reads as a coherent synthesis instead of a section-by-section concatenation. Focus on the big picture while preserving important nuance, proportional emphasis, and material conclusions. "
        "Accurately reflect uncertainty, limitations, or disagreements present in the source when they are important to understanding it. Be faithful to the source: do not invent facts, make unsupported inferences, or editorialize. "
        "Avoid padding, repetition, excessive detail, or quotations unless they are essential to convey the document's meaning.\n\n"
        + BASE_FORMAT
    ),
    "targeted_extraction": (
    ...
    "positional": (
        "You return the document content at a requested POSITION or STRUCTURAL LABEL.\n\n"
        "TASK: The user wants specific content identified by its PLACE/ORDER or its "
        "structural label rather than by topic (e.g. 'the last two', 'the first three', "
        "'the 5th', 'the one after X', or a named unit like 'section 9.2', 'clause 4.3', "
        "'chapter 3'). The context below is the document's content in its ORIGINAL ORDER. "
        "Then:\n"
        "- For an ORDINAL selector ('the last two', 'the 5th'), locate exactly the item(s) "
        "it selects, counting from the correct end and staying within the SAME kind of item "
        "the user named. Return ONLY those — never the whole list — reproduced IN FULL and "
        "VERBATIM, keeping each identifier and the source's order.\n"
        "- For a NUMBERED or TITLED SECTION/heading ('section 9.2', 'the Methods section'), "
        "find that exact heading in the ordered context and return that section's content IN "
        "FULL and VERBATIM — everything from the heading up to (but not including) the next "
        "heading of the same or higher level. Lead with the section's heading/title. Match "
        "the heading EXACTLY: do not confuse it with a similar-looking number elsewhere (e.g. "
        "'9.2' as a section vs. '9.2' appearing inside body text or a value).\n"
        "- If the user instead wants something ABOUT the selected item/section — its answer, "
        "solution, value, or explanation — first identify which one is selected and state it, "
        "then provide what was asked using only what the document gives for it.\n"
        "- If the requested position/section is not present in the context, say so plainly "
        "instead of returning a different one. Do not invent content that isn't there.\n\n"
        + BASE_FORMAT
    ),
    "comparison": (
